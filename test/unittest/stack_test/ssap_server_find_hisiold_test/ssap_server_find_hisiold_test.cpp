...
TEST_F(IT_SSAP_SERVER_FIND, FIND_PRIMARY_SERVICE_RSP_HISI_OLD)
{
    // step1：注册AppId
    int32_t appId = -1;
...
TEST_F(IT_SSAP_SERVER_FIND, FIND_PRIMARY_SERVICE_BY_STD_UUID_RSP_HISI_OLD)
{
    // step1：注册AppId
    int32_t appId = -1;
    NLSTK_Errcode_E ret = TestEnvInit(&appId, &g_serviceParam);
    ...
    // step3: 校验发送报文是否符合预期
    EXPECT_TRUE(TEST_DTAP_CompareLastPkt(rspPkt6Old, sizeof(rspPkt6Old)));
...
TEST_F(IT_SSAP_SERVER_FIND, FIND_PRIMARY_SERVICE_BY_CUSTOM_UUID_RSP_MIX_HISI_OLD)
{
    // step2：注册AppId
    int32_t appId = -1;
    ...
    // step1: 模拟服务端接收到FIND_STRUCTURE_REQ报文，服务端会根据操作码(0x06)进入对应处理函数SSAPS_FindReqHandle
    // 这里是在混合模式下查找自定义16字节UUID的属性
    DTAP_Data_Info_S dtapInfo = {.lcid = g_lcid};
    ...
    // step4: 校验发送报文是否符合预期
    EXPECT_TRUE(TEST_DTAP_CompareLastPkt(rspPkt5Old, sizeof(rspPkt5Old)));
...
TEST_F(IT_SSAP_SERVER_FIND, FIND_PROPERTY_BY_CUSTOM_UUID_RSP_MIX_HISI_OLD)
{
    // step1：注册AppId
    int32_t appId = -1;
    NLSTK_Errcode_E ret = TestEnvInit(&appId, &g_customUuidServiceParam);
    ...
    // step1: 模拟服务端接收到FIND_STRUCTURE_REQ报文，服务端会根据操作码(0x06)进入对应处理函数SSAPS_FindReqHandle
    // 这里是在混合模式下查找自定义16字节UUID的首要服务，并且设备是老设备
    DTAP_Data_Info_S dtapInfo = {.lcid = g_lcid};
    ...
    // step4: 校验发送报文是否符合预期
    EXPECT_TRUE(TEST_DTAP_CompareLastPkt(rspPkt4Old, sizeof(rspPkt4Old)));

    ret = NLSTK_SsapServerRemoveService(g_appId, g_serviceInTestCase->handle);
    EXPECT_EQ(ret, NLSTK_ERRCODE_SUCCESS);

    SDF_BuffFree(tmp);
}

TEST_F(IT_SSAP_SERVER_FIND, FIND_PROPERTY_BY_STD_UUID_RSP_HISI_OLD)
{
    // step1：注册AppId
    int32_t appId = -1;
    NLSTK_Errcode_E ret = TestEnvInit(&appId, &g_serviceParam);
    ...
    // step1: 模拟服务端接收到FIND_STRUCTURE_REQ报文，服务端会根据操作码(0x06)进入对应处理函数SSAPS_FindReqHandle
    // 这里是在混合模式下查找标准2字节UUID的首要服务，并且设备是老设备
    DTAP_Data_Info_S dtapInfo = {.lcid = g_lcid};
    ...
    // step4: 校验发送报文是否符合预期
    EXPECT_TRUE(TEST_DTAP_CompareLastPkt(rspPkt3Old, sizeof(rspPkt3Old)));
