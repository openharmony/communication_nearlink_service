...
{
    return TEST_SDF_CreateVectorByCapacity(SDF_VECTOR_DEFAULT_CAPACITY, traits);
}
...
static bool TEST_VectorGrow(SDF_Vector_S *vector)
{
...
void TEST_SDF_VectorRemoveLast(SDF_Vector_S *vector)
{
    ...
    TEST_SDF_VectorRemove(vector, vector->size - 1);
}
...
{
    return TEST_SDF_VectorFindFirstByStartIndex(vector, func, args, 0, index);
}
