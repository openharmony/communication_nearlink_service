...
NLSTK_Errcode_E NLSTK_SmRegAlgoFuncs(NLSTK_SmCryptoAlgoFuncs_S *funsIn)
{
    ...
    algos->sha256HashFunc = funsIn->sha256HashFunc;
    return NLSTK_ERRCODE_SUCCESS;
}
...
{
    SmSLink_S *slink = SmFindSLinkByLcid(lcid);
    NLSTK_CHECK_RETURN(slink != NULL, false, "[SM] Auth: cannot find slink by lcid, lcid: %u.", lcid);
    return slink->curStateIndex == SM_STATE_FULL || slink->curStateIndex == SM_STATE_ENCP;
}

bool SmIsSLinkEncryptComplete(uint16_t lcid)
{
    SmSLink_S *slink = SmFindSLinkByLcid(lcid);
    NLSTK_CHECK_RETURN(slink != NULL, false, "[SM] Encrypt: cannot find slink by lcid, lcid: %u.", lcid);
    return slink->curStateIndex == SM_STATE_FULL;
}
