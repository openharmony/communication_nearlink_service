...
static std::atomic<bool> g_hiviewUeFlag{true};
...
{
    HILOGI("DftUeStateChanged: %{public}d", isHiviewUeOn);
    g_hiviewUeFlag.store(isHiviewUeOn);
}
...
void DftManagerReport(DftEventEnum eventId, const DftParamC params[], size_t size)
{
    ...
    if (!g_hiviewUeFlag.load()) {
        HILOGD("HiviewUe state is off");
        return;
    }
