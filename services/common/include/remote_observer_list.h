...
{
    HILOGD("RemoteObserverList<T>::Register called");
    std::lock_guard<std::mutex> lock(lock_);
    ...
    if (observer->AsObject()->IsProxyObject() && !observer->AsObject()->AddDeathRecipient(dr)) {
...
{
    HILOGD("RemoteObserverList<T>::ObserverDeathRecipient::ObserverDeathRecipient called");
}
