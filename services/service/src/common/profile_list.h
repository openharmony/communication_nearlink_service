...
public:
     ...
     * @since 6
     */
    T Get(const SleTransport transport, const std::string &name)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    std::map<std::string, T> *GetProfiles(const SleTransport transport)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    void SetProfile(const SleTransport transport, const std::string &name, const T &data)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    bool Contains(const SleTransport transport, const std::string &name)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    bool Find(const SleTransport transport, const std::string &name, T &data)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    bool IsEmpty(const SleTransport transport)
    {
        std::lock_guard<std::mutex> lock(lock_);
     ...
     * @since 6
     */
    int Size(const SleTransport transport)
    {
        std::lock_guard<std::mutex> lock(lock_);
...
private:
    std::mutex lock_ = {};
    std::map<SleTransport, std::map<std::string, T>> profiles_ = {};
    SLE_DISALLOW_COPY_AND_ASSIGN(ProfilesList);
};
