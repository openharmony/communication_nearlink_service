...
namespace OHOS {
namespace Nearlink {
bool NearlinkSleAdvertiserSettings::Marshalling(Parcel &parcel) const
{
    ...
    if (!parcel.WriteUint8(linkRole_)) {
        return false;
    }
    if (!parcel.WriteUint8(primaryFrameType_)) {
        return false;
    }
    return true;
}

NearlinkSleAdvertiserSettings *NearlinkSleAdvertiserSettings::Unmarshalling(Parcel &parcel)
{
    NearlinkSleAdvertiserSettings *settings = new (std::nothrow) NearlinkSleAdvertiserSettings();
...
bool NearlinkSleAdvertiserSettings::WriteToParcel(Parcel &parcel)
{
    return Marshalling(parcel);
}

bool NearlinkSleAdvertiserSettings::ReadFromParcel(Parcel &parcel)
{
    ...
    std::string address = "";
    if (parcel.ReadString(address) && address.size() == RawAddress::SLE_ADDRESS_BYTE_LEN) {
    ...
    if (!parcel.ReadUint8(linkRole_)) {
        return false;
    }
    if (!parcel.ReadUint8(primaryFrameType_)) {
        return false;
    }
    return true;
}
