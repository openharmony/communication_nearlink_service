...
private:
    ...
    // 动作返回前，超时时间 = SA 加载超时 + taskTimeout_；动作返回后按 taskTimeout_ 重新计时，
    // 避免 SA 加载耗时长时，等待状态变化的超时时间也被一并拉长
    ...
    NlErrCode FinishSwitchAction(NearlinkSwitchEvent switchEvent, uint32_t actionId, NlErrCode ret);
    ...
    void OnTaskTimeout(uint32_t actionId);
    // 设置动作超时任务（已有旧任务则先取消），调用方须持有 nearlinkSwitchEventMutex_ 锁
    void SetTaskTimeout(uint32_t actionId, uint64_t delayUs);
    ...
    // 当前动作编号：动作启动、结束时 +1；超时任务和动作返回各自携带发起时的编号，
    // 与当前值不一致说明该动作已结束或被新动作取代，不再处理
