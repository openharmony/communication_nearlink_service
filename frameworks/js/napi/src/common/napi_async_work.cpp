    ...
    bool needCallback)
{
    ...
    asyncCallback->env_ = env;
    if (napi_create_promise(env, &asyncCallback->deferred_, &asyncCallback->promise_) != napi_ok) {
         HILOGE("failed to create promise.");
         return nullptr;
     }
    auto napiAsyncWork = std::make_shared<NapiAsyncWork>(env, asyncWork, asyncCallback, asyncWorkContext, needCallback);
...
void NapiAsyncWork::Info::Complete(void)
{
    ...
    if (needCallback && (errCode == NL_NO_ERROR)) {
        ...
        auto func = [asyncWorkWptr]() {
            ...
            if (asyncWorkSptr->GetAsyncWorkContext()) {
                NapiHaManager::GetInstance().ReportEvent(asyncWorkSptr->GetAsyncWorkContext()->apiName,
                    asyncWorkSptr->GetAsyncWorkContext()->beginTime, NL_ERR_TIMEOUT);
            }
    ...
    if (napiAsyncWork->napiAsyncCallback_) {
        ...
        if (napiAsyncWork->GetAsyncWorkContext()) {
            NapiHaManager::GetInstance().ReportEvent(napiAsyncWork->GetAsyncWorkContext()->apiName,
                napiAsyncWork->GetAsyncWorkContext()->beginTime, errCode);
        }
        napiAsyncWork->napiAsyncCallback_->CallFunction(errCode, object);
    }
}

void NapiAsyncWork::Run()
{
    napi_value resource = nullptr;
    napi_create_string_utf8(env_, "napiAsyncWork", NAPI_AUTO_LENGTH, &resource);

    NapiAsyncWork::Info *info = new (std::nothrow) NapiAsyncWork::Info();
    if (info == nullptr) {
...
{
    if (context_ == nullptr) {
        HILOGE("context_ is nullptr");
        return;
    }
    if (!needCallback_.load()) {
        HILOGE("Unsupported in no needCallback mode");
        if (context_->isNeedHaReport) {
            NapiHaManager::GetInstance().ReportEvent(context_->apiName,
                context_->beginTime, NL_ERR_INTERNAL_ERROR);
        }
        return;
    }
    ...
    auto func = [errCode, nativeObj, asyncWorkPtr = shared_from_this()]() {
        if (asyncWorkPtr && asyncWorkPtr->napiAsyncCallback_) {
            HILOGD("asycallback in");
            if (asyncWorkPtr->GetAsyncWorkContext()) {
                NapiHaManager::GetInstance().ReportEvent(asyncWorkPtr->GetAsyncWorkContext()->apiName,
                    asyncWorkPtr->GetAsyncWorkContext()->beginTime, errCode);
            }
            asyncWorkPtr->napiAsyncCallback_->CallFunction(errCode, nativeObj);
        }
    };
    DoInJsMainThread(env_, std::move(func), context_->apiName);
}
...
napi_value NapiAsyncWork::GetRet(void)
{
    ...
    return napiAsyncCallback_->GetPromiseRet();
}

void NapiAsyncWork::TimeoutCallback()
{
    HILOGI("enter");
    CallFunction(NL_ERR_TIMEOUT, nullptr);
}

std::shared_ptr<AsyncWorkContext> NapiAsyncWork::GetAsyncWorkContext() const
{
    return context_;
}
